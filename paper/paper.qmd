...
-   **Population (`population`)**: Variable provides an abbreviated description
    of the respondent group, usually indicating their voting status
    (e.g., "lv" for likely voters or "rv" for registered voters).
-   **State (`state`)**: The U.S. state where the poll was conducted or focused,
    if applicable.
...
-   $\beta_0$ is the intercept term, representing the baseline level of
    support.
-   $\beta_1, \beta_2, \beta_3, \beta_4$ are the coefficients for each predictor,
...
By incorporating these weights, we adjust the model’s estimates to give more importance to recent, credible, and representative polls, enhancing
...
This section examines the relationship between national polling trends, pollster effects, population demographics, and other factors with respect to the predicted support levels for Kamala Harris and Donald Trump in the 2024 U.S. Presidential Election. Using a dataset comprising polling data from various states and organizations, we applied both unweighted and weighted linear regression models to identify the key predictors influencing each candidate's support. Below, we present the results of our models along with their implications.
...
## Model Results and Interpretation {#sec-result-explain}

The linear regression models constructed for predicting support for Kamala Harris and Donald Trump in the 2024 U.S. Presidential Election utilized both unweighted and weighted configurations. As shown in **Table 1** and **Table 2**, the intercepts for Harris and Trump, estimated at 45.10 and 59.28 respectively in the unweighted models, represent their baseline support levels when all predictors are at reference values. The high \( R^2 \) values of 0.738 for Harris’s unweighted model and 0.860 for her weighted model, along with similar values of 0.635 and 0.890 for Trump, indicate that a significant portion of the variation in support can be explained by the model's predictors. The relatively low Root Mean Square Error (RMSE) values further suggest that the models provide reasonably accurate predictions of support levels.

The results underscore the significance of **national_poll** and **pollster** as key predictors for both candidates' support, highlighting how national trends and pollster differences impact the reported support levels. The **population** predictor further reveals that demographic and regional characteristics contribute meaningfully to support variations, especially in battleground states. These findings enhance our understanding of the dynamics influencing voter sentiment, with methodological and regional factors proving essential in shaping support for each candidate.
...
@fig-support-trend1 and @fig-support-trend2 illustrate the predicted support for Kamala Harris and Donald Trump from August 1, 2024, to October 27, 2024, using both unweighted and weighted linear regression models. In the unweighted model, we see that Trump's predicted support generally hovers between 45% and 50%, while Harris's support ranges from 35% to 45%. Both candidates' support levels appear relatively stable across the polling period, with minor fluctuations. This model, which treats all polls equally, suggests a consistent lead for Trump, albeit within a fairly close range, as shown by the overlapping grey points representing individual poll results.

The weighted model refines these predictions by applying weights based on recency and sample size, which reduces the influence of older or smaller polls. This approach results in smoother trend lines for both candidates, with less apparent variability compared to the unweighted model. Trump's support remains around the mid-40% range, while Harris’s support stays in the high 30% range. The weighting emphasizes more recent and larger polls, likely providing a more accurate reflection of current public opinion. Overall, both models consistently indicate a lead for Trump, with the weighted model slightly stabilizing the predictions.
...
@fig-support-state and @fig-support-state2 compare the predicted support for Kamala Harris and Donald Trump in key battleground states (Arizona, Georgia, Michigan, Nevada, North Carolina, Pennsylvania, and Wisconsin) for the 2024 U.S. election. @fig-support-state uses an unweighted multiple linear regression model, while @fig-support-state2 applies a weighted model, giving greater influence to polls with higher weights based on recency and sample size. In both figures, Trump’s support, represented by red lines, and Harris’s support, shown in blue, show close competition across most states, with slight variations in trends across the polling period from August to October 2024.

The weighted model in @fig-support-state2 introduces subtle shifts, with larger circles indicating polls with greater influence on the predicted trend. This approach captures more recent or higher-quality polls, offering a potentially more accurate representation of voter sentiment shifts over time. The close alignment of support lines for both candidates suggests tight races in these states, which are crucial for the election outcome. The use of weighted and unweighted models highlights the importance of data weighting in enhancing the reliability of electoral forecasts in dynamic political environments.
```{r}
#| echo: false
#| warning: false
#| message: false
...
@fig-support-pollster1 and @fig-support-pollster2 depict the predicted support for Kamala Harris and Donald Trump in the 2024 U.S. election, from August 1, 2024, to October 27, 2024, using a weighted multiple linear regression model. The red line shows fluctuations in Trump’s support, generally between 40% and 55%, with some noticeable peaks and dips, indicating variability over time. In contrast, Harris’s support, represented by the blue line, remains more stable, consistently ranging around 40% to 50%, suggesting less volatility in voter sentiment for her.

Pollsters such as AtlasIntel, Emerson, Beacon/Shaw, Siena/NY, and YouGov are represented by different colors, with circle sizes reflecting weights based on factors like recency and sample size. Larger circles correspond to higher-weighted polls, indicating they had a greater influence on the predictions. The model incorporates national poll data, adjustments for pollster effects, and population considerations, allowing for a refined prediction approach. This approach ensures that more recent and larger polls play a more significant role, capturing shifts in voter sentiment as the election approaches.
```{r}
#| echo: false
#| warning: false
#| message: false
