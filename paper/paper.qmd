---
title: "Based on polling as of 27 October 2024, Harris expects to win on 49.2%, leads Trump by 2% in 2024 US election"
...
harris_filtered <- harris_data |>
  filter(state %in% selected_states, as.Date(end_date) >= recent_date_cutoff)|>mutate(candidate = "Harris")
trump_filtered <- trump_data |>
  filter(state %in% selected_states, as.Date(end_date) >= recent_date_cutoff)|>mutate(candidate = "Trump")
...
new_data_harris <- data.frame(
  ...
  national_poll = rep(c(0, 1), length(1000)),  # Create a sequence for national_poll from 0 to 1
  population = factor("lv", levels = levels(harris_data$population)),  # Use the mean population or any relevant value
  sample_size = seq(min(0),max(2300),length.out=1000)
)|>
  mutate(
    ...
    recency = as.numeric(difftime(as.Date("2024-11-05"), as.Date("2024-08-01") + (1:1000), units = "days")),  # Adjust for the sequence
...
#| fig-cap: Predicted Support Trends for Kamala Harris and Donald Trump using fitted model
...
recent_date_cutoff <- as.Date("2024-05-01")
...
Harris model: RMSE of 3.47, indicating an average prediction error of
around 3.47 percentage points.\
Trump model: RMSE of 4.39, showing a slightly higher prediction error on
average.\

R-squared: Represents the proportion of the variance in the response
variable explained by the model (higher is better).\
Harris model: R-squared of 0.346, meaning the model explains about 34.6%
of the variance in Harris's polling data.\
Trump model: R-squared of 0.346 as well, indicating similar explanatory
...
Harris model: MAE of 2.50, meaning that, on average, the predictions are
off by 2.5 percentage points.\
Trump model: MAE of 3.13, indicating slightly less precise predictions
...
Model Fit: Both models explain roughly 34.6% of the variance in their
...
#| fig-cap: "Diagnostics of model using residual vs fitted plot and norm Q-Q plot\-Support for Harris"
...
@fig-dia3 shows residuals plotted against fitted values. Similar to the
