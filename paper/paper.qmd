...
Harris, offering understanding into the elements that shape voter preferences
...
the estimation is forecasting the percentage of Kamala’s vote share
based on an aggregated polling data using a multiple linear regression
model. Our goal is to track shifts in public opinion over time, offering
a clearer pattern of voter’s choice and potential election outcomes.
...
predictor, we aim to drawn significant conclusions about their
...
margins, having trustworthy understanding into voter preferences can
...
research. @sec-a provide external data detail, @sec-b provide
model detail, @sec-c provide a investigation for pollster Siena College's
methodology, and  @sec-d provide a idealized methodology for polling the poll.
...
essential for managing larger datasets [@citearrow]. We use ggplot2 [@citeggplot2] to visualize the analysis of data. In addition to the core packages, we employed several specialized libraries to facilitate specific analytical tasks. The modelsummary package enabled us to generate clean, interpretable model summaries, streamlining the reporting of regression and other statistical results [@citemodelsummary]. RColorBrewer provided color palettes for visualizations, enhancing clarity and visual appeal in our plots [@citeRColorBrewer]. The future package supported parallel processing, optimizing the runtime of intensive computations, especially beneficial in model training [@citefuture]. Finally,for resampling and bootstrapping techniques, we utilized the boot package, which allowed us to assess model stability and reliability [@citeboot1;@citeboot2]. caret facilitated model training and validation, offering a unified interface for cross-validation and performance metrics [@citecaret]. Our coding practices and file organization were informed by the structure outlined
in @tellingstories.
...
biases and mitigate random error. This aggregation process employs
...
valuable understanding into voter behavior and preferences.
...
when the survey period concluded. This date provides necessary context for
the poll results, as public opinion may change over time in response to
events, campaign actions, or other influencing factors. In our analysis, we only choose the polls with end date after July 21 2024, which Kamala Harris first declared in the election.

### Recency {#sec-recency}
```{r}
#| label: fig-recency
#| fig-cap: Distribution of the recency of the polls after July 21 2024
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: tbl-category
#| tbl-cap: Summary Statistics of categorical variable population and national_pull
#| echo: false
#| warning: false
#| message: false
...
National polls is another newly created variable in our dataset, represented by a binary
indicator (1 for polls at national level, 0 otherwise). This distinction is important because
...
grades. By limiting the maximum value of support at 80%, outliers and extreme variations are minimized, resulting in a clearer and more interpretable visualization.
...
ggplot(harris_data %>% filter(pct <= 80), aes(x = end_date, y = pct, color = factor(numeric_grade))) +
  geom_point(alpha = 0.7) +
...
Residuals vs fitted diagram to diagnostic the model. These procedures
...
utilized both unweighted and weighted configurations.The model summary of support for Harris is shown in @tbl-summary1, support for Trump is shown in @tbl-summary2. As shown in
...
unweighted model and 0.892 for her weighted model, along with similar
...
@fig-support-trend illustrate the predicted support for Kamala Harris
...
#| message: false
#| eval: true
#| fig-cap: "Harris Kamala leads Donald Trump an average 2% in Predicted Support in the 2024 US Election (From August 1, 2024, to November 2, 2024)"
...
lines, and Harris’s support, shown in blue, show close competition
across most states, with slight variations in trends across the polling
period from August to November 2024.
...
leads in Michigan, and Pennsylvania.  Harris also hold advantage in Wisconsin but facing tight competition. Overall, without weighting, polls contribute equally, creating more
volatility.
...
@fig-support-pollster depict the predicted support for Kamala Harris and
...
Quinnipiac favor Harris, while Emerson, AtlasIntel, and Beacon/Shaw
...
brings the state’s significance into sharp relief. Neveda, meanwhile,
...
of granularity, future analyses can provide deeper understanding into voter behavior and help
...
information by setting entries marked as NA to National, and create a
national_poll indicator, assigning a value of 1 for value in state equals to National and 0 for others. Dates are standardized using the lubridate
package to facilitate accurate time-based analysis.

Recency are calculated based on the days elapsed since the
...
Model Fit: Harris' model explain roughly 63% of the variance in their
respective datasets. Trump's models explain roughly 66% of the variance
...
@fig-stanareyouokay-1 and @fig-stanareyouokay-3 are residual plots of un-weighted model for Harris
...
@fig-stanareyouokay-2 and @fig-stanareyouokay-4 is are Q-Q plots of un-weighted and weighted model plot for Harris support. It shows most residuals fall along the line, especially in the
...
@fig-dia-1 and @fig-dia-3 shows the residuals plotted against the fitted values for the
unweighted model for Trump's Support. It shows that the residuals are generally spread
around zero with no clear pattern for both model, suggesting that the model is well-specified. 

@fig-dia-2 and @fig-dia-4 shows the Q-Q plot for the unweighted and weighted model for Trump's Support, showing that
...
how Siena addresses non-response and review the strengths and weaknesses
...
polling results, offering valuable understanding into voter behavior and
election dynamics.
...
allocate resources more efficiently and gain deeper understanding into
...
-   Social desirability bias typically stems from the desire of
    ...
    others, resulting in respondents not demonstrateing their actual
...
understanding into voter preferences and behaviors.

## Survey Design

The survey is designed to capture essential understanding into voting
...
We have defined the bias we want to solve in @sec-pollster.
