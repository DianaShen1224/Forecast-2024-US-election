---
title: "Based on polling as of 27 October 2024, Harris expects to win on xxx%, leads Trump by xx% in 2024 US election"
...
reflect voter sentiment [@nytimes].
...
1.  **Use of Weighted Model**: The weighted model is essential for
    accurately reflecting voter sentiment. By incorporating weights, we
    adjust for the reliability of the polling data, ensuring that more
    credible polls have a greater influence on the estimates.
...
harris_data <- harris_data %>%
  mutate(
    ...
    predicted_support = predict(model_harris_weighted, newdata = .),
    candidate = "Harris"
  )
...
trump_data <- trump_data %>%
  mutate(
    recency_weight = exp(-recency * 0.1),
    combined_weight = recency_weight * sample_size_weight * poll_frequency_weight * pollster_quality_weight,
    predicted_support = predict(model_trump_weighted, newdata = .),
    candidate = "Trump"
  )
...
#### Plot Support Trends by State with Reduced Date Labels ####
# Define states of interest
selected_states <- c("Pennsylvania", "Nevada", "North Carolina", "Wisconsin", "Michigan", "Georgia", "Arizona")

# Filter data for selected states
combined_data <- combined_data %>% filter(state %in% selected_states)

# Create the faceted plot with limited x-axis labels
ggplot(combined_data, aes(x = as.Date(end_date), y = predicted_support, color = candidate, size = combined_weight)) +
  geom_point(alpha = 0.6) +
  scale_color_manual(values = c("Harris" = "#1f77b4", "Trump" = "#d62728")) +  # Blue for Harris, Red for Trump
  scale_size_continuous(range = c(1, 4)) +  # Control point size range for readability
  scale_x_date(date_labels = "%b %d", date_breaks = "1 month") +  # Show only the first day of each month
  labs(title = "Support Trends for Harris and Trump Across States",
       x = "Date", y = "Support (%)", color = "Candidate") +
  facet_wrap(~ state, ncol = 2, scales = "free_y") +  # Facet by state with free y-scale
  theme_minimal(base_size = 12) +
  theme(
    legend.position = "top",
    strip.text = element_text(size = 12, face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1, size = 8),  # Slight tilt for x-axis labels
    panel.grid.major = element_line(size = 0.2, linetype = "dotted", color = "gray80"),
...
In this data-cleaning process, we focus on refining raw polling data to
...
# Sample indices for training and testing sets for Harris data
train_index_harris <- sample(seq_len(nrow(harris_data)), size = 0.8 * nrow(harris_data))

# Split the data into training and testing sets
train_data_harris <- harris_data[train_index_harris, ]
test_data_harris <- harris_data[-train_index_harris, ]

# Convert pollster to factor in the training dataset
train_data_harris$pollster <- factor(train_data_harris$pollster)

# Get levels from the training dataset
pollster_levels <- levels(train_data_harris$pollster)

# Set the same levels for pollster in testing data
test_data_harris$pollster <- factor(test_data_harris$pollster, levels = pollster_levels)

#### Fit the Unweighted Linear Regression Model for Harris ####
model_harris_unweighted <- lm(pct ~ national_poll + pollster + population, 
                             data = train_data_harris)

# Predict on test data for unweighted model
predictions_unweighted <- predict(model_harris_unweighted, newdata = test_data_harris)

# Calculate RMSE for unweighted model
rmse_unweighted <- sqrt(mean((predictions_unweighted - test_data_harris$pct)^2, na.rm = TRUE))
cat("Harris Unweighted Model RMSE:", rmse_unweighted, "\n")
