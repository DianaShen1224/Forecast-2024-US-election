...
Research by Gelman and King (1993) underscores that while polls can
...
Additionally, Erikson and Wlezien (2008) highlight the importance of
...
indicator (1 for national polls, 0 otherwise). This distinction is important because
...
of overall public opinion, and thus improve the accuracy of our predictive model.

## Population {#population}

The “population” variable in the dataset represents the status of each
entry, categorizing participants as “lv” (likely voters) or “rv”
(registered voters). They represent different levels of participation
and potential voting behavior. Likely voters are individuals who are not
only registered but are statistically more likely to vote based on historical
data or survey responses. In contrast, registered voters include all
individuals who are eligible to vote, regardless of their likelihood
of participation. 
...
estimated at 45.10 and 59.28 respectively in the unweighted models,
represent their baseline support levels when all predictors are at
reference values. The high ( R\^2 ) values of 0.738 for Harris’s
unweighted model and 0.860 for her weighted model, along with similar
values of 0.635 and 0.890 for Trump, indicate that a significant portion
...
support of Harris by 3.262%, decrease the support for Trump by 7.943%.
...
Trump in this group compared to likely voters. Specifically, the support
of registered voters is 2.1095 (unweighted model) and 1.0322 (weighted
...
and Donald Trump from August 1, 2024, to October 27, 2024, using both
...
support generally hovers between 45% and 50%, while Harris's support
ranges from 35% to 45%. Both candidates' support levels appear
relatively stable across the polling period, with minor fluctuations.
This model, which treats all polls equally, suggests a consistent lead
for Trump, albeit within a fairly close range, as shown by the
overlapping grey points representing individual poll results.
...
around the mid-40% range, while Harris’s support stays in the high 30%
range. The weighting emphasizes more recent and larger polls, likely
providing a more accurate reflection of current public opinion. Overall,
both models consistently indicate a lead for Trump, with the weighted
model slightly stabilizing the predictions.
...
#| message: false
#| eval: true
#| fig-cap: "Harris Kamala leads Donald Trump an average 5% in Predicted Support in the 2024 US Election (From August 1, 2024, to October 27, 2024)"
...
period from August to October 2024.

In the @fig-support-state-1 , support for Kamala Harris and Donald Trump
fluctuates widely across battleground states, with Harris holding slight
leads in Pennsylvania and North Carolina but facing tight competition
overall. Without weighting, polls contribute equally, creating more
volatility.
...
Pennsylvania, North Carolina, and Nevada, where she shows a clearer
...
#| fig-subcap: ["Unweighted","Weighted"]
#| layout-ncol: 2
#| fig-cap: "Harris is likely to lead Trump in Pennsylvania, North Carolina, and Nevada by Model Prediction"
...
ggplot(combined_data_total,aes(x = end_date,y=pct, colour = candidate)) +
  geom_point(alpha = 0.1)+ 
  geom_line(data = combined_predictions_filtered,aes(x=end_date,y = predicted_unweighted), size = 0.5)+ # Smoothed trend line
...
Donald Trump in the 2024 U.S. election, from August 1, 2024, to October
27, 2024, using a weighted multiple linear regression model. The red
line shows fluctuations in Trump’s support, generally between 40% and
55%, with some noticeable peaks and dips, indicating variability over
time. In contrast, Harris’s support, represented by the blue line,
remains more stable, consistently ranging around 40% to 50%, suggesting
less volatility in voter sentiment for her.

Major Pollsters are represented by different colors, with circle sizes
reflecting weights based on factors like recency and sample size. Larger
...
of granularity, future analyses can provide deeper insights into voter behavior and help
...
# Additional data details {#first-data-details}
...
assumption. However, some points at the head deviate, indicating
...
trend without significant systematic bias. 
...
unweighted model, there are deviations at the heads, though they appear
...
around zero with no clear pattern, suggesting that the model is well-specified. 
...
some points at the head deviate, indicating potential outliers or
...
with a theoretical normal distribution. Here, the residuals almost fully
align with the theoretical line,suggest that our data is almost normally distributed.
...
polling results, offering valuable insights into voter behavior and
election dynamics.
