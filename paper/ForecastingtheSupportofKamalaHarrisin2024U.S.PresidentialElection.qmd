---
title: "Forecasting the Support of Kamala Harris in 2024 U.S. Presidential Election using a Model-Based Approach"
subtitle: "Kamala Harris expects to win on 47.5%, leads Donald Trump by about 1% in 2024 US election in Popular Vote based on Poll-of-Polls Data and Linear Modeling."
author: 
  - Diana Shen
  - Jinyan Wei
  - Jerry Yu
...
```{r}
#| label: fig-recency
#| fig-cap: The distribution of the Poll Recency since July 21, 2024, highlights a strong focus on recent polling data, with the highest frequency of polls occurring within the first few days.
#| echo: false
#| warning: false
#| message: false
...
@fig-numericgrade displays the distribution of numeric grades assigned to various pollsters. The numeric grade is a metric that evaluates the quality or reliability of a pollster, considering factors such as methodology, historical accuracy, sample size, and pollster reputation. In this chart, we observe that the frequency of numeric grades peaks around 2.75 and 3.00, with a notable concentration at these values. This suggests that, after excluding lower-quality polls, a significant number of surveys are conducted by highly rated pollsters. Fewer polls have grades below 2.5, indicating a relatively lower occurrence of polls from less reliable pollsters in this dataset. This distribution highlights the emphasis on high-quality pollsters, ensuring a more reliable and consistent source of polling data for subsequent analysis.

```{r}
#| label: fig-numericgrade
#| fig-cap: The distribution of the Numeric Grade of the Polls shows that surveys are conducted by highly rated pollsters, which ensures the reliability of the analysis.
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: tbl-category
#| tbl-cap: Summary statistics of the categorical variables population and national_pull reveal that the majority of the polls are state-level (68.3%) and focus primarily on likely voters (75%).
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: fig-state
#| fig-cap: Count of Polls by the US state shows that Polling Distribution Prioritizes National Sentiment and High-Impact Battleground States.
#| echo: false
#| warning: false
#| message: false
...
```{r}
#| label: fig-pollster
#| fig-cap: Frequency of Polls Highlights Siena/NYT as Leading Pollster, Followed by YouGov and Emerson.
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: Trend Analysis of Kamala Harris Support from August to October Shows Upward Trends by Emerson and Beacon/Shaw, with Siena/NYT Showing Decline.
#| echo: false
#| warning: false
#| message: false
...
ggplot(filtered_data, aes(x = end_date, y = pct, color = pollster)) +
  ...
  labs(y = "Support for Harris (%)", x = "End Date of the Poll") +
  theme_classic() +
...
#| fig-cap: Harris's Support Trends Stable Around 50%, with Minor Fluctuations Across Pollster Numeric Grades, Especially in Polls with Numeric Grade 2.5.
#| echo: false
#| warning: false
#| message: false
...
ggplot(harris_data, aes(x = end_date, y = pct, color = factor(numeric_grade))) +
  ...
  labs(y = "Support for Harris (%)", x = "End Date of the Poll", color = "Numeric Grade") +
  scale_color_viridis_d() +
  theme_classic() +
  theme(legend.position = "bottom") +
  coord_cartesian(ylim = c(0, 100))
```
@tbl-harrissupport shows Harris' average support across states in the 2024 U.S. Presidential Election. She leads in Democratic-leaning states, with top support in Vermont (68.5%), Maryland (62.9%), and Massachusetts (61.2%). In key battlegrounds, Harris's support hovers around 50%, including Wisconsin (49.0%) and Pennsylvania (48.3%). Nationally, she averages 47.9%, with competitive support in swing states like Nevada (47.9%) and North Carolina (47.8%). However, in Republican-leaning states like Texas (44.3%) and Florida (44.2%), her support is lower. This distribution highlights Harris’s strength in Democratic regions and mixed prospects in competitive areas.
```{r}
#| label: tbl-harrissupport
#| tbl-cap: Harris leads in Democratic-leaning states, showing strong support in reliably blue areas. In pivotal swing states, she maintains competitive support in Wisconsin and Pennsylvania, reflecting her appeal in regions important for a potential electoral victory. However, in traditionally Republican strongholds, her support is lower. Nationally, Harris averages 47.9%.
#| echo: false
#| warning: false
#| message: false

# Calculate average support for Kamala Harris in each state
harris_data_state <- harris_data|>
  group_by(state)|>
  summarize(`Average Support (%)` = round(mean(pct, na.rm = TRUE), 1)) |>
  arrange(desc(`Average Support (%)`))|>rename(`State` = state) 

# Display the result as a table
kable(harris_data_state)
...
weighting scheme integrates factors like poll recency and sample size, which are
inspired by the New York Times' methodology [@nyt_polling_averages] for polling averages.
...
-   **Recency Weight**: Applied through an exponential decay function,
    ...
    poll was conducted, and 0.1 is the decay rate of the poll. This ensures that more recent data carries more
...
model validation and diagnostics, which are detailed in @sec-b. We
...
estimated at 44.70% and 51.87% respectively in the unweighted models, while in weighted models, it's estimated at 45.2% and 54.8%,
...
#| message: false
#| eval: true
#| tbl-cap: "Summary of Multiple Linear Regression Model Predicting Trump Support Based on Polling Data since 21 July 2024 (Harris first declared) to 2 November 2024"
#| label: tbl-summary2
msummary(
  list("Unweighted" = trump_unweighted_model, "Weighted" = trump_weighted_model),
  fmt = fmt_significant(3),coef_omit="state|pollster",notes = "Note. All models include a control for state and pollsters. The reference level of the state is Alaska, the reference level of population is likely voter, and the reference level of pollsters is ABC/Washington Post.")
```
...
#| message: false
#| eval: true
#| fig-cap: "Harris Kamala leads Donald Trump an average of 1% in Predicted Support in the 2024 US Election (From August 1, 2024, to November 2, 2024)."
...
ggplot(combined_data_total,aes(x = end_date,y=pct, colour = candidate,size = combined_weight)) +
  ...
  labs(x = "End Date of the Poll",
       ...
       color = "Candidate",
       size="Weight",
       caption = "Lines are weighted by recency_weight * sample_size_weight. Points are polls.\nModel: Candidate Support Percentage = national_poll + pollster + population + state\nLatest Poll: Nov 2; Source: FiftyThirtyEight") +
  scale_color_manual(values = c("Kamala Harris" = "blue", "Donald Trump" = "red"))  +
  theme_classic() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 45, hjust = 1),strip.background = element_rect(colour = "black", fill = "white")) +
...
#| fig-subcap: ["Unweighted","Weighted"]
#| layout-ncol: 1
#| fig-cap: "Harris is likely to lead Trump in Pennsylvania, Michigan, and Wisconsin by Model Prediction (From August 1, 2024, to November 2, 2024)."
...
ggplot(combined_data_total,aes(x = end_date,y=pct, colour = candidate))+
  geom_point(alpha = 0.1)+ 
  geom_smooth(data = combined_predictions_filtered,aes(x=end_date,y = predicted_unweighted),se = FALSE, size = 0.5)+
  facet_wrap(~ state,ncol = 4) +
  labs(
       x = "End Date of the Poll",  # Change x-axis label to End Date
       y = "Predicted Support (%)",
       color = "Candidate",
       caption = "Points are polls.\nModel: Candidate Support Percentage = national_poll + pollster + population + state\nLatest Poll: Nov 2; Source: FiftyThirtyEight") +
  scale_color_manual(values = c("Kamala Harris" = "blue", "Donald Trump" = "red")) +
  theme_classic() +
  theme(legend.position = "bottom",
...
ggplot(combined_data_total,aes(x = end_date,y=pct, colour = candidate,size = combined_weight)) +
  geom_point(alpha = 0.1)+ 
  geom_smooth(data = combined_predictions_filtered,aes(x=end_date,y = predicted_weighted),se = FALSE, size = 0.5)+ # Smoothed trend line
  facet_wrap(~ state,ncol = 4) +
  labs(
       x = "End Date of the Poll",  # Change x-axis label to End Date
       y = "Predicted Support (%)",
...
national_poll indicator, assigning a value of 1 for value in state equal to National and 0 for others. We also filter the population with only likely voters and registered voters. Dates are standardized using the lubridate
